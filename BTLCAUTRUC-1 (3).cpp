...
//                }
//                printf("\nGhi danh sach sinh vien vao file %s thanh cong!");
                pressAnyKey();
                break;
            case 0:
...
void nhapSV(SV a[], int n) {
    printLine(40);
    printf("\n Nhap sinh vien: ");
    nhapThongTinSV(a[n]);//&sv gan vo a[n]
    printLine(40);
}
//chinh sua thong tin sinh vien theo id
void capNhatThongTinSV(SV &sv) {
	cout << "\nNhap id: "; cin>>sv.id;
    cout << " Nhap ho va ten: "; fflush(stdin); getline(cin, sv.ten);
    cout << " Nhap gioi tinh( Nam hoac Nu ): "; gets(sv.gioiTinh);
    cout << " Nhap tuoi: "; cin >> sv.tuoi;
	do{
    	 cout << " Nhap diem Giai tich: "; cin >> sv.diemGiaitich;
...
//            &a[i].gioiTinh, &a[i].tuoi, &a[i].diemGiaitich, &a[i].diemLy, &a[i].diemCautruc, 
...
//    FILE * fp;
//    fp = fopen (fileName,"w");//con tro fp = fopen ham mo (tro toifileName ,w mo de ghi
//    for(int i = 0;i < n;i++){
//        fprintf(fp,"%5d%3%10f%10f%10f%10f%10s\n", a[i].id, a[i].ten,a[i].gioiTinh, //ghi vo file
//   fclose (fp); //dong file
//}          a[i].tuoi, a[i].diemGiaitich, a[i].diemLy, a[i].diemCautruc, a[i].diemTB, a[i].hocluc);
//    }
//   
void GhiFile(SV a[], int n){
